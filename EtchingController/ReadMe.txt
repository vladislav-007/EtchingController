================================================================================
    БИБЛИОТЕКА MICROSOFT FOUNDATION CLASS. Обзор проекта EtchingController
===============================================================================

Это приложение EtchingController автоматически создано с помощью мастера приложений. Это приложение демонстрирует основные принципы использования классов Microsoft Foundation; оно также может использоваться в качестве основы для написания собственных приложений.

В этом файле представлена сводка содержимого всех файлов, входящих в состав приложения EtchingController.

EtchingController.vcxproj
...
EtchingController.vcxproj.filters
...
EtchingController.h
    ...
    Он содержит прочие заголовки, относящиеся к проекту (включая Resource.h), а также объявляет класс приложения CEtchingControllerApp.

EtchingController.cpp
    Это основной исходный файл приложения, в котором содержится класс приложения CEtchingControllerApp.

EtchingController.rc
...
res\EtchingController.ico
    Файл значка, используемый в качестве значка приложения. Этот значок включен с помощью основного файла ресурсов EtchingController.rc.

res\EtchingController.rc2
...
res\Toolbar.bmp
    ...
    Начальная панель инструментов и строка состояния, созданные в классе CMainFrame. Измените этот точечный рисунок панели инструментов в редакторе ресурсов и обновите массив IDR_MAINFRAME TOOLBAR в файле EtchingController.rc, чтобы добавить кнопки панели инструментов.
/////////////////////////////////////////////////////////////////////////////
...
EtchingControllerDoc.h, EtchingControllerDoc.cpp — документ
    Эти файлы содержат класс CEtchingControllerDoc. Измените эти файлы, чтобы добавить особые данные документа и реализовать сохранение и загрузку файлов (посредством CEtchingControllerDoc::Serialize).
    Документ будет содержать следующие строки:
        Расширение файла:      ecf
        Идентификатор типа файла:        EtchingController.Document
        Заголовок главного фрейма:  EtchingController
        Имя типа документа:       EtchingControll
        Имя фильтра:         EtchingController Files (*.ecf)
        Новое краткое имя файла: EtchingControll
        Полное имя типа файла: EtchingController.Document

EtchingControllerView.h, EtchingControllerView.cpp — представление документа
    Эти файлы содержат класс CEtchingControllerView.
    Объекты CEtchingControllerView используются для просмотра объектов CEtchingControllerDoc.

res\EtchingControllerDoc.ico
    Файл значка, используемый в качестве значка дочерних окон MDI для класса CEtchingControllerDoc. Этот значок включен с помощью основного файла ресурсов EtchingController.rc.
...
StdAfx.h, StdAfx.cpp
    Эти файлы используются для построения файла предкомпилированного заголовка (PCH) с именем EtchingController.pch и файла предкомпилированных типов с именем StdAfx.obj.
